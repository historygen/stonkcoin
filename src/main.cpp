...
#if defined(NDEBUG)
# error "Stonkcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x80da1763c48c909cf37cb97c27eb8050a9a00b635db4dfccf9ebf53f78051cce");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Stonkcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Stonkcoin Signed Message:\n";
...
{
    // Stonkcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 12000); // Stonkcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Stonkcoin: 3.5 days
static const int64 nTargetSpacing = 3 * 60; // Stonkcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Stonkcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Stonkcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3bcce99f49b52ccb6adaa228b89f97f3d124dc1675a1cc5e3c312e37cc90615e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "United States Coronavirus Counts hits Million";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041cb6e291fcb1a275a5835541aac34b3c4e866c195ba65d77c7bbcee57990dd9159f69ecd99c035a8a7599e38ef74536e64e4f94eeb88ff1bcb72c5d86bf65134") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1588238946;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085720924;

        if (fTestNet)
        {
            block.nTime    = 1588238909;
            block.nNonce   = 385641820;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x51163c0fb2072f7e958290ad1cfff4085d285b50fc1b44c0afb9d3557145cb34"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xba, 0xd3 }; // Stonkcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StonkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StonkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static StonkcoinMiner(CWallet *pwallet)
{
    printf("StonkcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("stonkcoin-miner");
    ...
    try { loop {
        ...
        printf("Running StonkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("StonkcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StonkcoinMiner, pwallet));
}
